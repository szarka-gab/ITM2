...
public class ImageMetadataGenerator 
{
    ...
    */
    protected ImageMedia processImage( File input, File output, boolean overwrite ) throws IOException, IllegalArgumentException
    {
        ...
        // set orientation
        ImageIcon imgicon = new ImageIcon(input.getPath());
        ...
        // set pixel size
        media.setPxsize(img.getColorModel().getPixelSize());
        // set transparency
        media.setTransparency(img.getColorModel().getTransparency());
        // set number of (color) components  
        media.setColorcomponents(img.getColorModel().getComponentSize().length);
        ...
        for (int k = 0; k < h; k++) {
	        for (int j = 0; j < w; j++) {
	          int pixel = img.getRGB(j, k);
	        	  ...
	        	  if(red>green&&red>blue) r++;
	                if(red<green&&green>blue) g++;
	                if(red<blue&&blue>green) b++;
         ...
         if(r>g&&r>b) media.addTag("red");
         if(r<g&&g>b) media.addTag("green");
         if(r<b&&b>g) media.addTag("blue"); 
        
        if(r==g&&r==b) {    media.addTag("red");
		media.addTag("green");
		media.addTag("blue");}
        
        else if(r==b) {  media.addTag("red");
        			media.addTag("blue");}
        else if(r==g) {  media.addTag("red");
        			media.addTag("green");}
        else if(b==g) {  media.addTag("blue");
					media.addTag("green");}
        ...
        // store meta data
        System.out.println(media.getHeight()+"x"+media.getWidht());
        media.serializeObject();
        int n = url.toString().lastIndexOf("/");
        String name = url.toString().substring(n+1);
        File outputfl = new File(output+"/"+name+".txt");
        media.writeToFile(outputfl);
